...
#             props_df = props_df.loc[(props_df["area"]<25) & (props_df["area"]>5) & (props_df["solidity"]>0.98)]
            ...
            if num_features>0:
                ...
                for i in range(len(props_df)):
                    ...
                    fit_results = self.gmask_fit(spot_patch, xy_input=np.array(props_df.loc[i,'center_of_mass_coords'])-np.array([y0,x0]), fit=False)
...
#                 print(props_df)
    ...
    def gmask_fit(self, pic, xy_input=None, fit=False):
        ...
        y0 = 0.0
        F = 1.0 / (np.sqrt(2.0) * 1.6)  # This factor shows up repeatedly in GMASK
