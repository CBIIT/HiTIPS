...
#                                 im1_rot = rotate(img_patch, angle_1, preserve_range=True)
                        ...
                        #                     imputed_channles=[list_elem for list_elem in spot_channels if chnl!=list_elem]
                        #                     for im_ch in imputed_channles:

                        #                         imputed_patch_spot_intensity = large_rot_spot_img_stack[str(int(im_ch))][int(current_spot_loc[0]), int(current_spot_loc[1]), stack_ind]
                        #                         imputed_norm_spot_intensity = imputed_patch_spot_intensity/(imputed_patch_intensity_mean+0.000001)

                        #                         rows[-1]["imputed_ch"+str(im_ch)+"_intensity_mean"] = imputed_patch_intensity_mean
                        #                         rows[-1]["imputed_ch"+str(im_ch)+"_spot_intensity"] = imputed_patch_spot_intensity
                        #                         rows[-1]["imputed_ch"+str(im_ch)+"_normalized_spot_intensity"] = imputed_patch_spot_intensity/(imputed_patch_intensity_mean+0.000001)
                                    ...
                                    for lbl1 in np.unique(clusters)[np.unique(clusters)>0]: 
                                        ...
                                        for st_ind in range(anotated_large_rot_spot_img_stack.shape[2]):
                                            ...
                                            circ_for_draw = self.ImageAnalyzer.COORDINATES_TO_CIRCLE(spot_signal_df[[row_col_name, 
                                                                                                                     col_col_name]].to_numpy().astype(int)[st_ind].reshape(1,2),
                                    ...
                                    imwrite(self.cell_level_file_name(cell_tracking_folder, 'spot_image_patches', 'spot_img', '.tif', col, row, fov, id_,chnl), 
                                    ...
                                    imwrite(self.cell_level_file_name(cell_tracking_folder, 'unannotated_spot_image_patches', 'unannotated_spot_img', '.tif', col, row, fov, id_,chnl),
