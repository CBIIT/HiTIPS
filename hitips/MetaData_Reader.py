...
class ImageReader(object):
    ...
    def __init__(self, ControlPanel=None, inout_resource_gui=None, displaygui=None, analysisgui=None):
        ...
        if self.inout_resource_gui is not None:
    ...
    def ON_CLICK_LOADBUTTON(self):
        if self.inout_resource_gui.DeviceType.currentText() == 'CellVoyager':
            ...
            if self.fnames:
                ...
                self.Meta_Data_df = self.READ_FROM_METADATA(self.fnames[0])
                self.ControlPanel.Meta_Data_df = self.Meta_Data_df
        ...
        if self.inout_resource_gui.DeviceType.currentText() == 'MicroManager':
            options = QtWidgets.QFileDialog.Options()
            self.Data_dir = QtWidgets.QFileDialog.getExistingDirectory(None, caption="Select Data Directory", options=options)
            if self.Data_dir:
                self.Meta_Data_df = self.MICROMANAGER_READER(self.Data_dir)
                self.ControlPanel.Meta_Data_df = self.Meta_Data_df
                self.inout_resource_gui.DisplayCheckBox.setEnabled(True)
                self.analysisgui.setEnabled(True)
                PixPerMic_Text = 'Pixel Size = ' + "{:.2f}".format(float(self.Meta_Data_df["PixPerMic"].iloc[0])) + '\u03BC' + 'm'
    ...
    def ON_CLICK_LOADIMGBUTTON(self):
        ...
        self.fnames, _ = QtWidgets.QFileDialog.getOpenFileNames(None, 'Select Image Files...',
                                                                ...
                                                                options=options)
        if self.fnames:
            self.Meta_Data_df = self.LOAD_BIOFORMAT_DATA(self.fnames)
            self.ControlPanel.Meta_Data_df = self.Meta_Data_df
            self.inout_resource_gui.DisplayCheckBox.setEnabled(True)
            self.analysisgui.setEnabled(True)
            PixPerMic_Text = 'Pixel Size = ' + "{:.2f}".format(float(self.Meta_Data_df['PixPerMic'].iloc[0])) + '\u03BC' + 'm'
    ...
    def READ_FROM_METADATA(self, metadatafilename):
        ...
        for i in range(items_mrf.length):
            ...
            if self.displaygui is not None:
    ...
    def MICROMANAGER_READER(self, metadatadir):
        ...
        col_list = ['Time', 'Width', 'Height', 'PixelSize_um', 'Channel', 'FrameIndex', 'SlicePosition', 'Slice', 
                    'PositionIndex', 'PixelSizeUm', 'YPositionUm', 'XPositionUm', 'FileName', 'SliceIndex', 'ZPositionUm']
